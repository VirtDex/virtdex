...
const char * const BITCOIN_CONF_FILENAME = "VirtDex.conf";
const char * const BITCOIN_PID_FILENAME = "VirtDexd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "VirtDex" is a composite category enabling all VirtDex-related debug output
            if(ptrCategory->count(string("VirtDex"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VirtDexCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VirtDexCore
    // Mac: ~/Library/Application Support/VirtDexCore
    // Unix: ~/.VirtDexcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VirtDexCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/VirtDexCore";
#else
    // Unix
    return pathRet / ".VirtDexcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty VirtDex.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override VirtDex.conf
