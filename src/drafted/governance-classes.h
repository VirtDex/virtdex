 ...
 *
 *   // network
 *   CVirtDexNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.VirtDex.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CVirtDexNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CVirtDexNetwork(UniValue objIn)
    {
...
// // can be under: VirtDexNetwork
// //   -- signature requirements : Key1(User)
// class CVirtDexNetworkVariable : public CGovernanceObject
// {
// private:
